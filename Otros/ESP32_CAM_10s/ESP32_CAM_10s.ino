...
boolean signalDetected  = false;           // This is set when signal is received.
...
void codeCore0Task(void *parameter)
{
  ...
  signalDetected = true;  // Set here otherwise when first triggered by interrupt it doesn't get set.
  ...
  while(1)
  {
    ...
    if (currentMillis - lastSensorCheck > 10000)
    { 
    ...
    if (signalDetected && currentMillis - lastPictureTaken > FRAME_INTERVAL)
    {
...
void codeCore1Task(void *parameter)
{
  ...
  { 
    ...
    if (signalDetected && !fileOpen)
      startFile();


    // If there are frames waiting to be processed add these to the file.
    if (signalDetected && fileOpen && framesInBuffer() > 0)
      addToFile();
